...
# usage: gen_routing_benchmark.pl <nodes> <solution.pl> <net> <scl> <outputfile> <tile_size> <adjustment_factor%> <safe_guard_factor%> <mode=[2|3]>
...
if (!defined($nod_file) || !defined($sol_file) || !defined($net_file) || !defined($scl_file) || !defined($out_file) ||
    !defined($DEFAULT_TILE_SIZE) || !defined($ADJUSTMENT_FACTOR) || !defined($SAFE_GUARD) || !defined($mode)) {
    print "Usage: usage: gen_routing_benchmark.pl <nodes> <solution.pl> <net> <scl> <outputfile> <tile_size> <adjustment_factor%> <safe_guard_factor%> <mode=[2|3]>\n";
    exit(0);
}
...
if ($mode == 3) {
    ...
    print OUTFILE "vertical capacity\t0 $m2_capacity 0 $base_vert_capacity 0 $base_vert_capacity\n";
    print OUTFILE "horizontal capacity\t$m1_capacity 0 $base_hori_capacity 0 $base_hori_capacity 0\n";
    print OUTFILE "minimum width\t1 1 1 1 1 1\n";
    print OUTFILE "minimum spacing\t1 1 1 1 1 1\n";
    print OUTFILE "via spacing\t1 1 1 1 1 1\n";

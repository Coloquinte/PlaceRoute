...
NetWirelength NetWirelength::fromData(const std::vector<int> &cellSizes,
...
void NetWirelengthBuilder::push(const std::vector<int> &cells,
...
void NetWirelengthBuilder::push(const std::vector<int> &cells,
                                const std::vector<float> &offsets, float minPos,
                                float maxPos) {
  assert(cells.size() == offsets.size());
...
float NetWirelength::valueLSE(const xt::xtensor<float, 1> &pl,
                              float epsilon) const {
  float val = 0.0;
  for (const auto &bd : nets_) {
...
float NetWirelength::valueWA(const xt::xtensor<float, 1> &pl,
                             float epsilon) const {
  float val = 0.0;
  for (const auto &bd : nets_) {
...
xt::xtensor<float, 1> NetWirelength::gradHPWL(
    const xt::xtensor<float, 1> &pl) const {
...
xt::xtensor<float, 1> NetWirelength::gradLSE(const xt::xtensor<float, 1> &pl,
                                             float epsilon) const {
  xt::xtensor<float, 1> val = xt::zeros<float>({nbCells()});
  for (const auto &bd : nets_) {
...
xt::xtensor<float, 1> NetWirelength::gradWA(const xt::xtensor<float, 1> &pl,
                                            float epsilon) const {
  xt::xtensor<float, 1> val = xt::zeros<float>({nbCells()});
  for (const auto &bd : nets_) {
...
xt::xtensor<float, 1> NetWirelength::proximalStep(
    const xt::xtensor<float, 1> &pl, float step) const {
  xt::xtensor<float, 1> val = xt::zeros<float>({nbCells()});
  for (const auto &bd : nets_) {
...
NetWirelengthFixedSize::NetWirelengthFixedSize(int nbCells, int netSize,
                                               std::vector<int> pinCells,
                                               std::vector<float> pinOffsets) {
...
NetWirelengthFixedSize::NetWirelengthFixedSize(
    int nbCells, xt::xtensor<int, 2> pinCells,
    xt::xtensor<float, 2> pinOffsets) {
...
xt::xtensor<float, 2> NetWirelengthFixedSize::pinCoords(
    xt::xtensor<float, 1> pl) const {
  assert(pl.size() == nbCells());
  xt::xtensor<float, 2> cellCoords =
      xt::reshape_view(xt::index_view(pl, pinCells_), {nbNets(), netSize()});
...
float NetWirelengthFixedSize::valueLSE(xt::xtensor<float, 1> pl,
                                       float epsilon) const {
  auto pins = pinCoords(pl);
  return xt::sum(lseMax(pins, epsilon) + lseMax(-pins, epsilon))();
}

float NetWirelengthFixedSize::valueWA(xt::xtensor<float, 1> pl,
                                      float epsilon) const {
  auto pins = pinCoords(pl);
  return xt::sum(waMax(pins, epsilon) + waMax(-pins, epsilon))();
}

xt::xtensor<float, 1> NetWirelengthFixedSize::gradHPWL(
    xt::xtensor<float, 1> pl) const {
  auto pins = pinCoords(pl);
  return toCellGrad(maxGrad(pins) - maxGrad(-pins));
}

xt::xtensor<float, 1> NetWirelengthFixedSize::gradLSE(xt::xtensor<float, 1> pl,
                                                      float epsilon) const {
  auto pins = pinCoords(pl);
  return toCellGrad(lseMaxGrad(pins, epsilon) - lseMaxGrad(-pins, epsilon));
}

xt::xtensor<float, 1> NetWirelengthFixedSize::gradWA(xt::xtensor<float, 1> pl,
                                                     float epsilon) const {
  auto pins = pinCoords(pl);
  return toCellGrad(waMaxGrad(pins, epsilon) - waMaxGrad(-pins, epsilon));
}

xt::xtensor<float, 1> NetWirelengthFixedSize::proximalStep(
    xt::xtensor<float, 1> pl, float step) const {
  auto pins = pinCoords(pl);
  return toCellGrad(maxProximal(pins, step) - maxProximal(-pins, step));
}

xt::xtensor<float, 1> NetWirelengthFixedSize::lseMax(xt::xtensor<float, 2> pins,
                                                     float epsilon) const {
...
xt::xtensor<float, 1> NetWirelengthFixedSize::waMax(xt::xtensor<float, 2> pins,
                                                    float epsilon) const {
  xt::xtensor<float, 1> maxVal = xt::amax(pins, {1});
  // Normalize by the maximum and remove infinities for the product computation
  // later
  xt::xtensor<float, 2> normalized =
      xt::maximum(pins - xt::expand_dims(maxVal, 1), -1000.0f * epsilon);
  xt::xtensor<float, 2> expValue = xt::exp(normalized / epsilon);
  return maxVal + xt::sum(normalized * expValue, 1) / xt::sum(expValue, 1);
}

xt::xtensor<float, 2> NetWirelengthFixedSize::maxGrad(
    xt::xtensor<float, 2> pins) const {
...
xt::xtensor<float, 2> NetWirelengthFixedSize::lseMaxGrad(
    xt::xtensor<float, 2> pins, float epsilon) const {
  xt::xtensor<float, 1> maxVal = xt::amax(pins, {1});
  xt::xtensor<float, 2> normalized = pins - xt::expand_dims(maxVal, 1);
  xt::xtensor<float, 2> expValue = xt::exp(normalized / epsilon);
...
xt::xtensor<float, 2> NetWirelengthFixedSize::waMaxGrad(
    xt::xtensor<float, 2> pins, float epsilon) const {
  xt::xtensor<float, 1> maxVal = xt::amax(pins, {1});
  // Normalize by the maximum and remove infinities for the product computation
  // later
  ...
  return (numDerivative * xt::expand_dims(denum, 1) - num * denumDerivative) /
         xt::square(xt::expand_dims(denum, 1));
}

xt::xtensor<float, 2> NetWirelengthFixedSize::posProximal(
    xt::xtensor<float, 2> pins, float step) const {
  float penalty = 0.5 / step;
  xt::xtensor<float, 1> maxVal = xt::amax(pins, {1});
  xt::xtensor<float, 2> normalized = pins - xt::expand_dims(maxVal, 1);
  xt::xtensor<int, 2> sorted = xt::argsort(normalized, 1);
  xt::xtensor<float, 2> dest = xt::zeros<float>(pins.shape());
  // It is a pain to do this kind of stuff in xtensor, so let's go for the
  // simple loop
...
xt::xtensor<float, 2> NetWirelengthFixedSize::maxProximal(
    xt::xtensor<float, 2> pins, float step) const {
  return posProximal(pins, step) - pins;
}

xt::xtensor<float, 2> NetWirelengthFixedSize::maxProximal(
    xt::xtensor<float, 2> pins, xt::xtensor<float, 1> fixedPins,
    float step) const {
  xt::xtensor<float, 2> pos = posProximal(pins, step);
  // Stop the movement at the fixed pin
  xt::xtensor<float, 2> satPos =
      xt::maximum(pos, xt::expand_dims(fixedPins, 1));
  // Pins below the fixed pin do not move at all
  xt::xtensor<float, 2> finalPos = xt::minimum(pins, satPos);
  return finalPos - pins;
}

xt::xtensor<float, 1> NetWirelengthFixedSize::toCellGrad(
    xt::xtensor<float, 2> pinGrad) const {
...
NetWirelengthFixedSizeTerminals::NetWirelengthFixedSizeTerminals(
    int nbCells, int netSize, std::vector<int> pinCells,
    std::vector<float> pinOffsets, std::vector<float> minPos,
...
NetWirelengthFixedSizeTerminals::NetWirelengthFixedSizeTerminals(
    int nbCells, xt::xtensor<int, 2> pinCells, xt::xtensor<float, 2> pinOffsets,
    xt::xtensor<float, 1> minPos, xt::xtensor<float, 1> maxPos)
...
float NetWirelengthFixedSizeTerminals::valueHPWL(
    xt::xtensor<float, 1> pl) const {
  auto pinsMinMax = pinCoordsMinMax(pl);
  return xt::sum(xt::amax(pinsMinMax.second, {1}) -
                 xt::amin(pinsMinMax.first, {1}))();
}

float NetWirelengthFixedSizeTerminals::valueLSE(xt::xtensor<float, 1> pl,
                                                float epsilon) const {
  auto pinsMinMax = pinCoordsMinMax(pl);
  return xt::sum(lseMax(pinsMinMax.second, epsilon) +
                 lseMax(-pinsMinMax.first, epsilon))();
}

float NetWirelengthFixedSizeTerminals::valueWA(xt::xtensor<float, 1> pl,
                                               float epsilon) const {
  auto pinsMinMax = pinCoordsMinMax(pl);
  return xt::sum(waMax(pinsMinMax.second, epsilon) +
                 waMax(-pinsMinMax.first, epsilon))();
}

xt::xtensor<float, 1> NetWirelengthFixedSizeTerminals::gradHPWL(
    xt::xtensor<float, 1> pl) const {
  auto pinsMinMax = pinCoordsMinMax(pl);
  return toCellGradMinMax(maxGrad(pinsMinMax.second) -
                          maxGrad(-pinsMinMax.first));
}

xt::xtensor<float, 1> NetWirelengthFixedSizeTerminals::gradLSE(
    xt::xtensor<float, 1> pl, float epsilon) const {
  auto pinsMinMax = pinCoordsMinMax(pl);
  return toCellGradMinMax(lseMaxGrad(pinsMinMax.second, epsilon) -
                          lseMaxGrad(-pinsMinMax.first, epsilon));
}

xt::xtensor<float, 1> NetWirelengthFixedSizeTerminals::gradWA(
    xt::xtensor<float, 1> pl, float epsilon) const {
  auto pinsMinMax = pinCoordsMinMax(pl);
  return toCellGradMinMax(waMaxGrad(pinsMinMax.second, epsilon) -
                          waMaxGrad(-pinsMinMax.first, epsilon));
}

xt::xtensor<float, 1> NetWirelengthFixedSizeTerminals::proximalStep(
    xt::xtensor<float, 1> pl, float step) const {
  auto pins = pinCoords(pl);
  return toCellGrad(maxProximal(pins, maxPins_, step) -
                    maxProximal(-pins, -minPins_, step));
}

std::pair<xt::xtensor<float, 2>, xt::xtensor<float, 2> >
NetWirelengthFixedSizeTerminals::pinCoordsMinMax(
    xt::xtensor<float, 1> pl) const {
  auto pins = pinCoords(pl);
  xt::xtensor<float, 2> minPins =
      xt::concatenate(xtuple(pins, xt::expand_dims(minPins_, 1)), 1);
  xt::xtensor<float, 2> maxPins =
      xt::concatenate(xtuple(pins, xt::expand_dims(maxPins_, 1)), 1);
  return std::make_pair(minPins, maxPins);
}

xt::xtensor<float, 2> NetWirelengthFixedSizeTerminals::pinCoordsAll(
    xt::xtensor<float, 1> pl) const {
  auto pins = pinCoords(pl);
  return xt::concatenate(
      xtuple(pins, xt::expand_dims(minPins_, 1), xt::expand_dims(maxPins_, 1)),
      1);
}
...
xt::xtensor<float, 2> NetWirelengthFixedSizeTerminals::pinOffsetsAll() const {
  return xt::concatenate(xtuple(pinOffsets(), xt::expand_dims(minPins_, 1),
                                xt::expand_dims(maxPins_, 1)),
                         1);
}

xt::xtensor<float, 1> NetWirelengthFixedSizeTerminals::toCellGradMinMax(
    xt::xtensor<float, 2> pinGrad) const {
  return toCellGrad(
      xt::view(pinGrad, xt::all(), xt::range(xt::placeholders::_, -1)));
}

void NetWirelengthFixedSizeBuilder::push(const std::vector<int> &cells,
                                         const std::vector<float> &offsets) {
...
void NetWirelengthFixedSizeTerminalsBuilder::push(
    const std::vector<int> &cells, const std::vector<float> &offsets,
    float minPin, float maxPin) {
  assert(cells.size() == netSize_);
  assert(offsets.size() == netSize_);
  pinCells_.insert(pinCells_.end(), cells.begin(), cells.end());
  pinOffsets_.insert(pinOffsets_.end(), offsets.begin(), offsets.end());
...
MatrixBuilder MatrixBuilder::createStar(const NetWirelength &topo,
                                        xt::xtensor<float, 1> pl, float epsilon,
                                        float relaxation, bool b2bScale) {
  MatrixBuilder bd(topo.nbCells());
...
void MatrixBuilder::extendStar(const NetWirelengthFixedSize &topo,
  ...
  for (int i = 0; i < topo.nbNets(); ++i) {
    if (topo.netSize() == 2) {
      ...
      addMovingPin(cells(i, 0), cells(i, 1), offsets(i, 0), offsets(i, 1),
                   weight);
    } else {
      int newCell = nbCells_ + nbSupps_;
      nbSupps_++;
      ...
      for (int j = 0; j < topo.netSize(); ++j) {
        if (j == amnt(i) || j == amxt(i)) {
          ...
          float weight =
              1.0f /
              std::max(halfDiameter + relaxation * halfDiameter, epsilon);
          float anchor = j == amnt(i) ? relaxation * halfDiameter
                                      : -relaxation * halfDiameter;
          ...
          addMovingPin(cells(i, j), newCell, offsets(i, j), pos - pinMed(i),
                       weight);
        }
      }
    }
...
void MatrixBuilder::addQuadraticPenalty(xt::xtensor<float, 1> target,
                                        xt::xtensor<float, 1> forceConstants) {
...
void MatrixBuilder::addLinearPenalty(xt::xtensor<float, 1> target,
                                     xt::xtensor<float, 1> pl,
                                     xt::xtensor<float, 1> forces,
  ...
  for (size_t c = 0; c < target.size(); ++c) {
    forceConstants[c] =
        forces[c] / std::max(cutoffDistance, std::abs(target[c] - pl[c]));
  }
  addQuadraticPenalty(target, forceConstants);
}

MatrixBuilder MatrixBuilder::createB2B(const NetWirelength &topo,
                                       xt::xtensor<float, 1> pl,
                                       float epsilon) {
  MatrixBuilder bd(topo.nbCells());
  bd.initial_ = std::vector<float>(pl.begin(), pl.end());
...
void MatrixBuilder::extendB2B(const NetWirelengthFixedSize &topo,
                              xt::xtensor<float, 1> pl, float epsilon) {
  const auto coords = topo.pinCoords(pl);
  const auto &cells = topo.pinCells();
  const auto &offsets = topo.pinOffsets();
  ...
  for (int i = 0; i < topo.nbNets(); ++i) {
    ...
    for (int j = 0; j < topo.netSize(); ++j) {
      ...
      if (j == amn) continue;
      weight =
          factor / std::max(std::abs(coords(i, j) - coords(i, amn)), epsilon);
      addMovingPin(cells(i, j), cells(i, amn), offsets(i, j), offsets(i, amn),
                   weight);
      if (j == amx) continue;
      weight =
          factor / std::max(std::abs(coords(i, j) - coords(i, amx)), epsilon);
      addMovingPin(cells(i, j), cells(i, amx), offsets(i, j), offsets(i, amx),
                   weight);
    }
  }
}

void MatrixBuilder::extendB2B(const NetWirelengthFixedSizeTerminals &topo,
                              xt::xtensor<float, 1> pl, float epsilon) {
  const auto coords = topo.pinCoordsAll(pl);
  const auto cells = topo.pinCellsAll();
  const auto offsets = topo.pinOffsetsAll();
  ...
  for (int i = 0; i < topo.nbNets(); ++i) {
    ...
    for (int j = 0; j < topo.netSize() + nbFixed; ++j) {
      ...
      weight =
          factor / std::max(std::abs(coords(i, j) - coords(i, amn)), epsilon);
      addPin(cells(i, j), cells(i, amn), offsets(i, j), offsets(i, amn),
             weight);
      if (j == amx) continue;
      weight =
          factor / std::max(std::abs(coords(i, j) - coords(i, amx)), epsilon);
      addPin(cells(i, j), cells(i, amx), offsets(i, j), offsets(i, amx),
             weight);
    }
  }
}

void MatrixBuilder::addMovingPin(int c1, int c2, float offs1, float offs2,
                                 float weight) {
  if (c1 == c2) return;
  assert(c1 >= 0);
  assert(c2 >= 0);
...
void MatrixBuilder::addPin(int c1, int c2, float offs1, float offs2,
                           float weight) {
  if (c1 == c2) return;
  if (c1 == -1) {
...
xt::xtensor<float, 1> NetWirelength::starSolve(const xt::xtensor<float, 1> &pl,
                                               float epsilon, float relaxation,
                                               bool b2bScale) const {
  MatrixBuilder bd =
      MatrixBuilder::createStar(*this, pl, epsilon, relaxation, b2bScale);
  return bd.solve();
}

xt::xtensor<float, 1> NetWirelength::starSolvePenalized(
    const xt::xtensor<float, 1> &pl, float epsilon,
    const xt::xtensor<float, 1> &targets, const xt::xtensor<float, 1> &forces,
    float cutoffDistance, float relaxation, bool b2bScale) const {
  MatrixBuilder bd =
      MatrixBuilder::createStar(*this, pl, epsilon, relaxation, b2bScale);
...
xt::xtensor<float, 1> NetWirelength::b2bSolve(const xt::xtensor<float, 1> &pl,
                                              float epsilon) const {
  MatrixBuilder bd = MatrixBuilder::createB2B(*this, pl, epsilon);
  return bd.solve();
}

xt::xtensor<float, 1> NetWirelength::b2bSolvePenalized(
    const xt::xtensor<float, 1> &pl, float epsilon,
    const xt::xtensor<float, 1> &targets, const xt::xtensor<float, 1> &forces,

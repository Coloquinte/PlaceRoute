...
class DensityGrid {
 public:
  /**
   * Represent a rectangular group of bins in the grid; max coords are
   * non-inclusive
   */
  struct BinGroup {
    int minXCoord;
  ...
  /**
   * Initialize a grid from a circuit, and initialize the bins from the standard
   * cell height
   */
 ...
 private:
  ...
  /**
   * Obtain cleaned-up placement regions by removing any overlap between regions
   * and obstacles
   */
  // TODO

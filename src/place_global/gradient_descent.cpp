...
namespace {
void lineSearch(xt::xtensor<float, 1> start, xt::xtensor<float, 1> direction,
                float &step, float factor,
                const std::function<float(xt::xtensor<float, 1>)> &eval) {
  /**
   * Simple line search where we look for the minimum of the function; no
   * checking of Wolfe/Armijo conditions
   */
...
float computeValue(const NetWirelength &topo, xt::xtensor<float, 1> current,
                   DescentModel model, float smoothing) {
...
xt::xtensor<float, 1> descentDirection(const NetWirelength &topo,
                                       xt::xtensor<float, 1> current,
                                       DescentModel model, float smoothing) {
  if (model == DescentModel::HPWL) {
...
void reportFooter() { std::cout << std::endl; }

void report(const NetWirelength &topo, xt::xtensor<float, 1> pl,
            DescentModel model, int stepInd, float stepSize, float momentum,
            float smoothing, LineSearchType lineSearch, DescentType descent) {
...
xt::xtensor<float, 1> gradientDescentFixedStep(const NetWirelength &topo,
  ...
  for (int i = 0; i < nbSteps; ++i) {
    report(topo, x, model, i, stepSize, momentum, smoothing,
           LineSearchType::FixedStep, DescentType::GradientDescent);
    xt::xtensor<float, 1> z =
        y + stepSize * descentDirection(topo, x, model, smoothing);
    y = z + momentum * (z - x);
    x = z;
  }
...
xt::xtensor<float, 1> gradientDescent(const NetWirelength &topo,
                                      xt::xtensor<float, 1> initial,
                                      DescentModel model, int nbSteps,
                                      float momentum, float smoothing,
  ...
  for (int i = 0; i < nbSteps; ++i) {
    report(topo, x, model, i, initialStepSize, momentum, smoothing,
           LineSearchType::Armijo, DescentType::GradientDescent);
    xt::xtensor<float, 1> direction =
        descentDirection(topo, x, model, smoothing);
    lineSearch(x, direction, currentStep, stepVariation,
               [&](xt::xtensor<float, 1> w) -> float {
                 return computeValue(topo, w, model, smoothing);
               });
    xt::xtensor<float, 1> z = y + currentStep * direction;
    y = z + momentum * (z - x);
    x = z;
  }
  report(topo, x, model, nbSteps, initialStepSize, momentum, smoothing,
         LineSearchType::Armijo, DescentType::GradientDescent);
  reportFooter();
  return x;
}
...
xt::xtensor<float, 1> conjugateGradientDescent(const NetWirelength &topo,
  ...
  reportHeader();
  xt::xtensor<float, 1> x = initial;
  xt::xtensor<float, 1> prevSearchDirection =
      xt::zeros<float>({topo.nbCells()});
  xt::xtensor<float, 1> prevGradient = xt::zeros<float>({topo.nbCells()});
  float currentStep = initialStepSize;
  for (int i = 0; i < nbSteps; ++i) {
    report(topo, x, model, i, initialStepSize, 0.0, smoothing,
           LineSearchType::Armijo, DescentType::ConjugateGradientDescent);
    xt::xtensor<float, 1> gradient =
        -descentDirection(topo, x, model, smoothing);
    xt::xtensor<float, 1> searchDirection = -gradient;
    if (i > 0) {
      float beta = std::max(xt::sum(gradient * (gradient - prevGradient))() /
                                xt::sum(xt::square(prevGradient))(),
                            0.0f);
      searchDirection = -gradient + beta * prevSearchDirection;
    }
    lineSearch(x, searchDirection, currentStep, stepVariation,
               [&](xt::xtensor<float, 1> w) -> float {
                 return computeValue(topo, w, model, smoothing);
               });
  ...
  report(topo, x, model, nbSteps, initialStepSize, 0.0, smoothing,
         LineSearchType::Armijo, DescentType::ConjugateGradientDescent);
  reportFooter();
  return x;
}
...
xt::xtensor<float, 1> nesterovGradientDescent(const NetWirelength &topo,
  ...
  for (int i = 0; i < nbSteps; ++i) {
    report(topo, u, model, i, initialStepSize, 0.0, smoothing,
           LineSearchType::Armijo,
           DescentType::NesterovAcceleratedGradientDescent);
    xt::xtensor<float, 1> gradient =
        -descentDirection(topo, v, model, smoothing);
    lineSearch(v, -gradient, alpha, stepVariation,
               [&](xt::xtensor<float, 1> w) -> float {
                 return computeValue(topo, w, model, smoothing);
               });
  ...
  report(topo, u, model, nbSteps, initialStepSize, 0.0, smoothing,
         LineSearchType::Armijo,
         DescentType::NesterovAcceleratedGradientDescent);
  reportFooter();
  return u;

...
class NetWirelength {
 public:
  ...
  static NetWirelength fromData(const std::vector<int> &cellSizes,
  ...
  const std::vector<NetWirelengthFixedSizeTerminals> &terminalNets() const {
    return terminalNets_;
  }
  ...
  xt::xtensor<float, 1> gradLSE(const xt::xtensor<float, 1> &pl,
                                float epsilon) const;
  xt::xtensor<float, 1> gradWA(const xt::xtensor<float, 1> &pl,
                               float epsilon) const;
  xt::xtensor<float, 1> proximalStep(const xt::xtensor<float, 1> &pl,
                                     float step) const;
  xt::xtensor<float, 1> starStep(const xt::xtensor<float, 1> &pl,
                                 float epsilon) const;
  xt::xtensor<float, 1> b2bStep(const xt::xtensor<float, 1> &pl,
                                float epsilon) const;

  // Direct solutions
  xt::xtensor<float, 1> starSolve() const;
  xt::xtensor<float, 1> starSolve(const xt::xtensor<float, 1> &pl,
                                  float epsilon, float relaxation = 0.0,
                                  bool b2bScale = false) const;
  xt::xtensor<float, 1> starSolvePenalized(const xt::xtensor<float, 1> &pl,
                                           float epsilon,
                                           const xt::xtensor<float, 1> &targets,
                                           const xt::xtensor<float, 1> &forces,
                                           float cutoffDistance,
                                           float relaxation = 0.0,
                                           bool b2bScale = false) const;
  xt::xtensor<float, 1> b2bSolve(const xt::xtensor<float, 1> &pl,
                                 float epsilon) const;
  xt::xtensor<float, 1> b2bSolvePenalized(const xt::xtensor<float, 1> &pl,
                                          float epsilon,
                                          const xt::xtensor<float, 1> &targets,
                                          const xt::xtensor<float, 1> &forces,
...
/**
 * Representation of nets of a given size as 1D HPWL for optimization
 * algorithms.
 *
 * This allows us to use more efficient array operations and specialize for
 * fixed terminals
 */
class NetWirelengthFixedSize {
 public:
  NetWirelengthFixedSize(int nbCells, int netSize, std::vector<int> pinCells,
                         std::vector<float> pinOffsets);
  NetWirelengthFixedSize(int nbCells, xt::xtensor<int, 2> pinCells,
  ...
  xt::xtensor<float, 1> proximalStep(xt::xtensor<float, 1> pl,
                                     float step) const;
 ...
 protected:
  ...
  xt::xtensor<float, 2> lseMaxGrad(xt::xtensor<float, 2> pins,
                                   float epsilon) const;
  // Compute the gradient of the WA approximation of max on a 2D array
  xt::xtensor<float, 2> waMaxGrad(xt::xtensor<float, 2> pins,
                                  float epsilon) const;
  // Compute the proximal step of max on a 2D array, like a gradient step
  xt::xtensor<float, 2> maxProximal(xt::xtensor<float, 2> pins,
                                    float step) const;
  xt::xtensor<float, 2> maxProximal(xt::xtensor<float, 2> pins,
                                    xt::xtensor<float, 1> fixedPins,
                                    float step) const;
  // Compute the position after proximal step of max on a 2D array
  xt::xtensor<float, 2> posProximal(xt::xtensor<float, 2> pins,
                                    float step) const;
...
class NetWirelengthFixedSizeTerminals : NetWirelengthFixedSize {
 public:
  NetWirelengthFixedSizeTerminals(int nbCells, int netSize,
                                  std::vector<int> pinCells,
                                  std::vector<float> pinOffsets,
                                  std::vector<float> minPos,
                                  std::vector<float> maxPos);
  NetWirelengthFixedSizeTerminals(int nbCells, xt::xtensor<int, 2> pinCells,
                                  xt::xtensor<float, 2> pinOffsets,
                                  xt::xtensor<float, 1> minPos,
                                  xt::xtensor<float, 1> maxPos);
  ...
  std::pair<xt::xtensor<float, 2>, xt::xtensor<float, 2> > pinCoordsMinMax(
...
class NetWirelengthFixedSizeBuilder {
 public:
  NetWirelengthFixedSizeBuilder(int nbCells, int netSize)
      : nbCells_(nbCells), netSize_(netSize) {}
  bool empty() const { return pinCells_.empty(); }
  void push(const std::vector<int> &cells, const std::vector<float> &offsets);
  NetWirelengthFixedSize build() const {
    return NetWirelengthFixedSize(nbCells_, netSize_, pinCells_, pinOffsets_);
  }
...
class NetWirelengthFixedSizeTerminalsBuilder {
 public:
  NetWirelengthFixedSizeTerminalsBuilder(int nbCells, int netSize)
      : nbCells_(nbCells), netSize_(netSize) {}
  bool empty() const { return pinCells_.empty(); }
  void push(const std::vector<int> &cells, const std::vector<float> &offsets,
            float minPin, float maxPin);
  NetWirelengthFixedSizeTerminals build() const {
    return NetWirelengthFixedSizeTerminals(nbCells_, netSize_, pinCells_,
                                           pinOffsets_, minPins_, maxPins_);
  }
...
class MatrixBuilder {
 public:
  MatrixBuilder(int nbCells)
      : nbCells_(nbCells), nbSupps_(0), rhs_(nbCells), initial_(nbCells) {}
  ...
  void extendStar(const NetWirelengthFixedSize &topo, xt::xtensor<float, 1> pl,
                  float epsilon, float relaxation, bool b2bScale);
  void extendStar(const NetWirelengthFixedSizeTerminals &topo,
                  xt::xtensor<float, 1> pl, float epsilon, float relaxation,
                  bool b2bScale);
  void addQuadraticPenalty(xt::xtensor<float, 1> target,
                           xt::xtensor<float, 1> forceConstants);
  void addLinearPenalty(xt::xtensor<float, 1> target, xt::xtensor<float, 1> pl,
                        xt::xtensor<float, 1> forces, float cutoffDistance);

  // Bound to bound model, with local approximation from the current placement
  static MatrixBuilder createB2B(const NetWirelength &topo,
                                 xt::xtensor<float, 1> pl, float epsilon);
  void extendB2B(const NetWirelengthFixedSize &topo, xt::xtensor<float, 1> pl,
                 float epsilon);
  void extendB2B(const NetWirelengthFixedSizeTerminals &topo,
                 xt::xtensor<float, 1> pl, float epsilon);

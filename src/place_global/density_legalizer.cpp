...
DensityLegalizer::DensityLegalizer(std::vector<Rectangle> regions,
                                   int nbCells) {
  regions_ = regions;
...
float DensityLegalizer::distL2() const { return std::sqrt(distL2Squared()); }
...
void DensityLegalizer::report() const {
  ...
  std::cout << "Area (" << placementArea_.minX << ", " << placementArea_.maxX
            << ") x (" << placementArea_.minY << ", " << placementArea_.maxY
            << ")" << std::endl;
  /*
  std::cout << "X limits";
...
std::vector<std::vector<long long> > DensityLegalizer::binUsage() const {
  std::vector<std::vector<long long> > ret(nbBinsX(),
                                           std::vector<long long>(nbBinsY()));
  for (int i = 0; i < nbBinsX_; ++i) {
    for (int j = 0; j < nbBinsY_; ++j) {
      for (int c : binCells_[i][j]) {
...
void DensityLegalizer::rebisect(int x1, int y1, int x2, int y2,
  ...
  auto b = bisect(binX_[x1], binY_[y1], binX_[x2], binY_[y2],
                  binCapacity_[x1][y1], binCapacity_[x2][y2], cells, leg);
...
void DensityLegalizer::assign() {
  ...
  for (int c = 0; c < nbCells(); ++c) {
    ...
    int i = std::lower_bound(binLimitX_.begin(), binLimitX_.end(), x) -
            binLimitX_.begin();
    i = std::max(std::min(i, nbBinsX() - 1), 0);
    int j = std::lower_bound(binLimitY_.begin(), binLimitY_.end(), y) -
            binLimitY_.begin();
...
struct SplitArea {
  ...
  int nbBins() const { return (maxI - minI) * (maxJ - minJ); }
  ...
  std::pair<SplitArea, SplitArea> splitX(LegalizationModel model) const {
    ...
    auto p = leg.bisect(s1.x(), s1.y(), s2.x(), s2.y(), s1.capacity(),
                        s2.capacity(), cells, model);
    s1.cells = p.first;
    s2.cells = p.second;
